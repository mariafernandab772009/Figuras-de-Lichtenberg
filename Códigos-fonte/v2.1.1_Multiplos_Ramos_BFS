...
class Node(Point2D):
    # Classe construtora, inicializa as coordenadas do nó e seu valor na matriz
...
class Queue:
    ...
    # Enfileira um objeto do tipo 'nó'
    def enqueue(self, node):
        self.queue.append(node)
        self.size += 1
        return node

    # Desenfileira (remove o primeiro elemento da fila) e retorna o elemento removido
    def dequeue(self):
        if (self.size > 0):
...
class LeastResistance:
    ...
    # Encontra o valor mínimo entre os nós em torno do nó da raiz
    def findMin(self, node, minima):
        # Ordem de direção preferida de viagem
    ...
    def BFS_Traversal(self, root):
        ...
        while q.size > 0:
            ...
            # Se o algoritmo encontra uma entrada nula, ele termina
            else:
                found = 0
...
# ani.save('MultiploRamoBFS.gif', writer='pillow', fps=120, dpi=100)
plt.show()

# SALVA A ANIMAÇÃO
print("Salvando animação como MultiploRamoBFS.gif... Aguarde.")
ani.save('MultiploRamoBFS.gif', writer='pillow', fps=15)
